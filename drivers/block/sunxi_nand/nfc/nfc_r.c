...
const __s16 param0x30high[20][2] = {
	{0x00,0XF0},
	{0x0F,0XE0},
	{0x0F,0XD0},
	{0x0E,0XE0},
...
/* FIXME: dma_wait_mode is unused and may be removed. */
...
inline void nand_send_cmd_part(__u8 cmd)
{
	__u32 cfg;
...
inline void nand_send_cmd(__u8 cmd)
{
	__u32 cfg;
...
inline void nand_send_read_cmd(__u32 regcnt)
{
	__u32 cfg;
...
inline void hynix_send_otp_rrt_prefix(__u8 addr, __u8 data)
{
	__u32 cfg;
...
__s32 NFC_ReadRetry(__u32 chip, __u32 retry_count, __u32 read_retry_type)
{
	...
	/* Hynix read retry modes END */
		...
		if(retry_count==4) {
			...
			ret = _vender_set_param(param_intel, adr_intel, read_retry_reg_num);
		}
		ret =_vender_set_param(param, read_retry_reg_adr, read_retry_reg_num);
	}
