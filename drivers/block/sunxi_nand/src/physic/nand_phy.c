...
__s32 _read_single_page_1K(struct boot_physical_param *readop,__u8 dma_wait_mode)
{
	...
	if (SUPPORT_RANDOM) {
		...
		if (ret) {
			...
			ret = NFC_Read_1K(cmd_list, readop->mainbuf, sparebuf, dma_wait_mode, NFC_PAGE_MODE);
			if (ret)
				pr_warn("%s: NFC_Read_1K (non-randomised) error %d", __FUNCTION__, ret);
		}
	}
	else {
		ret = NFC_Read_1K(cmd_list, readop->mainbuf, sparebuf, dma_wait_mode, NFC_PAGE_MODE);
	...
	if (readop->oobbuf)
		MEMCPY(readop->oobbuf, sparebuf, 64);
...
/* FIXME: blatant code duplication */
__s32 _read_single_page_spare(struct boot_physical_param *readop,__u8 dma_wait_mode)
{
	...
	if(SUPPORT_READ_RETRY) {
    	...
    	if(k>0) {
    		PHY_DBG("[Read_single_page_spare] NFC_ReadRetry %d cycles, "
			"chip = %d, block = %d, page = %d, RetryCount = %d  \n",
		...
		if(SUPPORT_RANDOM) {
			...
			NFC_RandomDisable();
			if(ret == -ERR_ECC) {
				pr_err("%s non-RR random read ECC ERROR\n", __FUNCTION__);
				ret = NFC_Read_Spare(cmd_list, readop->mainbuf, sparebuf, dma_wait_mode , NFC_PAGE_MODE);
			}
		}
		else
